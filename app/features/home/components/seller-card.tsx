import { TypographyBase } from '@shared/components/typography/typography-base'
import { RoutesEnum } from '@routes'
import { Link } from 'react-router'

interface SellerCardProps {
  name: string
  content: string
  imageUrl: string
}

export function SellerCard({ name, content, imageUrl }: SellerCardProps) {
  return (
    <div className="relative lg:h-1/3 w-1/2 min-w-[350px] min-h-[450px] lg:min-h-[200px]">
      <div
        title={`Foto do Artesão ${name}`}
        style={{ backgroundImage: `url(${imageUrl})` }}
        className="bg-no-repeat bg-cover bg-center w-full lg:w-2/3 h-80 transform -scale-x-100"
      />
      <div className="absolute top-60 lg:top-8 w-11/12 lg:w-3/5 h-fit lg:h-72 lg:bottom-0 right-3.5 lg:right-0 bg-background text-secondary pt-6 pl-5 pr-5 pb-4 flex flex-col gap-5">
        <TypographyBase className="text-lg lg:text-xl font-medium">
          {name}
        </TypographyBase>
        <TypographyBase className="text-sm lg:text-md font-medium">
          {content}
        </TypographyBase>
        <Link
          className="underline decoration-2 underline-offset-4 text-foreground font-medium"
          to={RoutesEnum.HOME}
        >
          Ler mais
        </Link>
      </div>
    </div>
  )
}
